 ...
 * types A, C, G and T have impact factors of 1, 2, 3 and 4, respectively. You are 
 * going to answer several queries of the form: What is the minimal impact factor 
...
/*
 * Slow solution, will come back to this.
 */
...
class Solution {
    public int[] solution(String S, int[] P, int[] Q) {
        ...
        int index = 0;
        for(char nucleotide: S.toCharArray()) {
            switch(nucleotide) {
                case 'A':
                    impacts[index] = 1;
                    break;
                ...
                case 'T':
                    ...
                    break;
                default:
                    System.out.println("A, C, G, or T not found.");
                    break;
            }
            index++;
        }

        for(int query = 0; query < P.length; query++) {
            int startIndex = P[query];
            int endIndex = Q[query];
            int minImpact = 4;
            while(startIndex <= endIndex) {
                if(impacts[startIndex] < minImpact) {
                    minImpact = impacts[startIndex];
                }
                startIndex++;
            }
            queryResults[query] = minImpact;
        }
